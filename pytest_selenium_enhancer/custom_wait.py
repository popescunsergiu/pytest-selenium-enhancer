...
class CustomWait:
    ...
    def wait_for_element_present(self, by=By.XPATH, value=None, text=None, timeout=60):
        if text is not None:
            value = value % text
        wait = WebDriverWait(self.driver, timeout)
        return wait.until(EC.presence_of_element_located((by, value)))

    def wait_for_element_visible(self, by=By.XPATH, value=None, text=None, timeout=60):
        if text is not None:
            value = value % text
        wait = WebDriverWait(self.driver, timeout)
    ...
    def wait_for_child_element_visible(self, parent_element, by=By.XPATH, value=None, text=None, timeout=60):
        if text is not None:
            value = value % text
        wait = WebDriverWait(self.driver, timeout)
        return wait.until(CEC.visibility_of_child_element_located(parent_element, (by, value)))

    def wait_for_child_element_not_visible(self, parent_element, by=By.XPATH, value=None, text=None, timeout=60):
        if text is not None:
            value = value % text
    ...
    def wait_for_the_attribute_value(self, element, attribute, value, timeout=60):
        wait = WebDriverWait(self.driver, timeout)
        return wait.until(CEC.wait_for_the_attribute_value(element, attribute, value))

    def wait_for_the_attribute_contain_value(self, element, attribute, value, timeout=60):
        wait = WebDriverWait(self.driver, timeout)
